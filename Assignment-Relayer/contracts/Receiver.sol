...
contract Receiver {
    ...
    constructor() {
        ...
        domainSeparator = keccak256(
            abi.encode(
                ...
                keccak256(bytes("EIP712 Relayer Assignment")),
                keccak256(bytes("1")),
                getChainId(),
                address(this)
            )
    ...
    function executeMetaTransaction(
        ...
        for (uint256 i = 0; i < transactions.length; i++) {
            ...
            (address user, address targetAddress, address to, uint256 amount, uint256 nonce) = abi
                .decode(transaction, (address, address, address, uint256, uint256));
            ...
            bytes32 digest = keccak256(
                abi.encodePacked(
                    ...
                    domainSeparator,
                    keccak256(
                        abi.encode(TRANSACTION_TYPEHASH, user, targetAddress, to, amount, nonce)
                    )
                )
            );
